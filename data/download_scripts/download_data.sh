...
for i in ${files[@]}; do
    ...
    wget -q -O - "${download_path}" | tqdm --desc "Download" --bytes --unit_scale --total $total > "$(basename "${download_path}")"

    # Extract and delete zip
    n_files=$(unzip -l ${file_name} | tail -n 1 | awk '{print $2}')
    unzip -o ${file_name} -d "../raw" | tqdm --desc "Extract" --unit files --unit_scale --total $n_files > /dev/null
    rm -f ${file_name}
done